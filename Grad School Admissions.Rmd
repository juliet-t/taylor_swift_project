...
```{r}
df1 <- read.csv(here("data_raw", "Admission_Predict_Ver1.1.csv")) |> clean_names()
```
...
# Influence of Gre Scores on Admission Chances
```{r}
plot(gre, chance_admit, xlab = "Gre Scores", ylab = "Chance of Admission") 
title(main = "Gre Score's Correlation with Chance of Admission")
gre_lm <- lm(chance_of_admit ~gre_score, data = df1)
abline(gre_lm)
tidy(gre_lm)
...
```{r}
plot(cgpa, chance_admit, xlab = "CGPA Score", ylab = "Chance of Admission") 
...
# Influence of Research on Admission Chances
```{r}
ggplot(df1, aes(y = chance_of_admit, x = research)) + geom_bar(position='dodge', stat='identity')
res_lm <- lm(chance_of_admit ~reserach, data = df1)
