...
# Packages Needed: Install only if you do not have these packages already
...
cat("\nSample mean:", mean(cgpa))
cat("\nStandard Deviation:", sd(cgpa))
```
...
# Influence of GPA on Admission Chances
```{r}
plot(df2$gpa, chance_admit, xlab = "GPA Score", ylab = "Chance of Admission") 
title(main = "GPA Score's correlation with Chance of Admission")
gpa_lm <- lm(chance_of_admit ~gpa, data = df2)
abline(gpa_lm)
tidy(summary(gpa_lm))

cat(txt_cor, cor(df2$gpa, chance_admit)) 
cat("\nSample mean:", mean(df2$gpa))
cat("\nStandard Deviation:", sd(df2$gpa))
...
Rejection region: (-infinity, -1.96] U [1.96, infinity)
